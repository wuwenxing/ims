 ...
 */
@Component
@ActTaskType({ActTaskTypeEnum.REAL_TOTAL_CLOSE_LOT,ActTaskTypeEnum.DEMO_TOTAL_CLOSE_LOT})
public class TotalCloseLotTaskItemTemplate extends AbstractBaseTask {
	...
	public List<ImsTaskRecord> isFinishTask(ActTaskSettingVO actTask, String accountNo, String platform,
		...
		if(ActTaskTypeEnum.DEMO_TOTAL_CLOSE_LOT.getCode().equals(actTask.getTaskItemCode())){
		...
		//平仓累计手数
		BigDecimal totalCloseLots = BigDecimal.ZERO; //统计的总平仓次数
		//任务参数平仓手数
        BigDecimal taskCloseLots = BigDecimal.valueOf(Long.valueOf(actTask.getTaskItemVal())); 
		...
		if(CollectionUtils.isNotEmpty(tradeList)){
			for(int i=0;i<tradeList.size();i++){
				...
				totalCloseLots = totalCloseLots.add(ImsBigDecimalUtil.roundDown4(tradeList.get(i).getTradeLot()));
            	if(totalCloseLots.compareTo(taskCloseLots) >= 0){
                	//生成任务完成记录
    				imsTaskRecordList = builderTaskRecords(actTask, accountNo, platform, companyId, tradeDate,accType,taskCloseLots,totalCloseLots);
    				break;
                }
            }
        }
	 ...
	 * @param taskCloseLots  任务交易手数
	 * @param totalCloseLots  总交易手数
	 * @return List<ImsTaskRecord> 返回任务记录列表
	 */
	private List<ImsTaskRecord> builderTaskRecords(ActTaskSettingVO actTask,String accountNo, String platform,Long companyId,Date tradeDate,String accType,BigDecimal taskCloseLots,BigDecimal totalCloseLots){
		...
		for(ActTaskItemsVO item : items) {
			...
			imsTaskRecord.setTaskParamLot(taskCloseLots);
			imsTaskRecord.setFinishLot(totalCloseLots);
			imsTaskRecord.setItemAmount(item.getItemPrice());
			imsTaskRecord.setItemAmountUnit(item.getItemParamValUnit());
			imsTaskRecord.setNeedTradeLot(taskCloseLots);
