...
@Component
@ActSettleType({ActSettleTypeEnum.MANUAL})
public class ManualSettlementType extends AbstractSettlementType{

	private String SETTLE_REMARK = "手动清算" ;
	...
	public boolean doSettlement(ActAccountActiviStat stat) {
		...
		//没有完成要求的手数赠金回扣
		if(!cancelUnreleaseWithGold(stat.getActNo(), stat.getAccountNo(), stat.getAboutSettleTime(), ActSettleTypeEnum.MANUAL.getCode(), stat.getPlatform(), stat.getCompanyId())){
			return false ;
		}
		return true;
	}
