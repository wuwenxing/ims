...
*/
@Component
@IssueItemCategory({IssueItemCategoryEnum.MOBILECHARGE})
public class MobileChargeItemManager extends AbstractIssueItems{

	Logger logger = LoggerFactory.getLogger(MobileChargeItemManager.class);
	
	@Autowired
	private OnlineDubboService onlineDubboService;
	...
	public Boolean send(String recordNo,String itemType,String actNo,String accountNo,Long companyId) {
		...
		logger.info("send-->话费 【RechargeType:{},actNo:{},companyId:{}】", itemType, actNo, companyId);
		
		ImsPrizeRecord pr=imsPrizeRecordManager.findByRecordNo(recordNo, actNo);
		logger.info("send-->话费  发放记录不存在！请查看物品是否有效或数量是否足够！【RechargeType:{},actNo:{},companyId:{}】", itemType, actNo, companyId);
		try {
		String channel = "";
		...
		ApiRespResult<Map<String, Object>> result = onlineDubboService.send(pr.getCustMobile(), size, channel, "127.0.0.1", extMap, companyId);
