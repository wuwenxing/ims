...
*/
@Component
@IssueItemCategory({IssueItemCategoryEnum.MOBILEDATA})
public class MobileDataItemManger extends AbstractIssueItems{
	
	Logger logger = LoggerFactory.getLogger(MobileDataItemManger.class);
	
	@Autowired
	private FlowDubboService flowDubboService;
	...
	public Boolean send(String recordNo,String itemType,String actNo,String accountNo,Long companyId) {
		logger.info("send-->流量  【RechargeType:{},actNo:{},companyId:{}】", itemType, actNo, companyId);
		companyId=2l;
		ImsPrizeRecord pr=imsPrizeRecordManager.findByRecordNo(recordNo, actNo);
		if(pr==null) {
			logger.info("send-->流量  发放记录不存在！请查看物品是否有效或数量是否足够！【RechargeType:{},actNo:{},companyId:{}】", itemType, actNo, companyId);
			return false;
		}
		...
		if(StringUtil.isNullOrEmpty(pr.getCustMobile())){ //手机号为空，直接返回发放失败，并备注对应的信息
			logger.info("send-->手机号为空，流量发放失败!【RechargeType:{},actNo:{},companyId:{}】", itemType, actNo, companyId);
		...
		String remark = null;
		if("G".equals(pr.getItemAmountUnit())){
			size = Long.parseLong(size) * 1024 + "";
		}
		ApiRespResult<Map<String, Object>> result = flowDubboService.send(pr.getCustMobile(), size, channel, "127.0.0.1", extMap, companyId);
		String resBatchNo = null;
		//if(null != result && result.isOk()){//TODO  IMS 和API 成功CODE 未统一，待统一后处理
		if(null != result && result.getCode().equals("OK")){
			...
			logger.info("send-->调用第三方接口，流量发放失败!【RechargeType:{},actNo:{},companyId:{},msg:{}】", itemType, actNo, companyId,result.getMsg());
			pr.setGivedStatus(IssuingStatusEnum.issue_fail.getValue());
			remark = null == result ? "":result.getMsg();
		}
