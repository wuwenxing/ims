 ...
 */
@Component
@ActTaskType({ActTaskTypeEnum.REAL_TOTAL_CLOSE_TIMES,ActTaskTypeEnum.DEMO_TOTAL_CLOSE_TIMES})
public class TotalCloseTimesTaskItemTemplate extends AbstractBaseTask {
	...
	public List<ImsTaskRecord> isFinishTask(ActTaskSettingVO actTask, String accountNo, String platform, Long companyId) {
		...
		if(ActTaskTypeEnum.DEMO_TOTAL_CLOSE_TIMES.getCode().equals(actTask.getTaskItemCode())){
		...
		//平仓累计总数
		BigDecimal totalCloseTimes = BigDecimal.ZERO; //统计的总平仓次数
		//任务参数平仓数
		BigDecimal taskCloseTimes = BigDecimal.valueOf(Long.valueOf(actTask.getTaskItemVal())); 
		...
		if(CollectionUtils.isNotEmpty(tradeList)){
			for(int i=0;i<tradeList.size();i++){
            	...
            	totalCloseTimes = totalCloseTimes.add(BigDecimal.ONE);
            	tradeDate = tradeList.get(i).getTradeTime();
            	if(totalCloseTimes.compareTo(taskCloseTimes) >= 0){
                	//生成任务完成记录
    				imsTaskRecordList = builderTaskRecords(actTask, accountNo, platform, companyId, tradeDate,accType,taskCloseTimes,totalCloseTimes);
    				break;
                }
            }
        }
	 ...
	 * @param taskCloseTimes  任务交易次数
	 * @param totalCloseTimes  总交易次数
	 * @return List<ImsTaskRecord> 返回任务记录列表
	 */
	private List<ImsTaskRecord> builderTaskRecords(ActTaskSettingVO actTask,String accountNo, String platform,Long companyId,Date tradeDate,String accType,BigDecimal taskCloseTimes,BigDecimal totalCloseTimes){
		...
		for(ActTaskItemsVO item : items) {
			...
			imsTaskRecord.setTaskParamLot(taskCloseTimes);
			//imsTaskRecord.setFinishLot(totalCloseTimes);
			imsTaskRecord.setItemAmount(item.getItemPrice());
			imsTaskRecord.setItemAmountUnit(item.getItemParamValUnit());
			imsTaskRecord.setNeedTradeLot(taskCloseTimes);
