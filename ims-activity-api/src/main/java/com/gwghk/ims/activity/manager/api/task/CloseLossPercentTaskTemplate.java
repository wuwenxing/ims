...
*/
@Component
@ActTaskType({ActTaskTypeEnum.REAL_CLOSE_LOSS_PERCENT,ActTaskTypeEnum.DEMO_CLOSE_LOSS_PERCENT})
public class CloseLossPercentTaskTemplate extends AbstractBaseTask {
	...
	public List<ImsTaskRecord> isFinishTask(ActTaskSettingVO actTask, String accountNo, String platform, 
		...
		if(ActTaskTypeEnum.DEMO_CLOSE_LOSS_PERCENT.getCode().equals(actTask.getTaskItemCode())){
		...
		//取出上一次任务的完成时间点，从上次一次的时间开始查询,非循环性取活动的开始时间和结束时间
		...
		for(ActTradeRecord tradeRecord:tradeRecordList) {
			...
			//是否满足亏损条件
			if(tradeRecord.getTradeTime().compareTo(taskDate[0])>0 && tradeRecord.getProfit().doubleValue()<0 && BigDecimalUtil.add(tradeRecord.getProfit(), taskLoss).doubleValue() <= 0) {
				List<ActTaskItemsVO> items = actTask.getTaskItems() ;
                BigDecimal itemGiftAmount = BigDecimal.ZERO;//此次计算发放的赠金值 ,要求手数=itemGiftAmount/最高金额 *交易手数
				...
				for(ActTaskItemsVO item : items) {
					...
					//本次需要发放额度
					itemGiftAmount = BigDecimalUtil.roundDown4(tradeRecord.getProfit().multiply(new BigDecimal(-1)).multiply(itemParam).divide(new BigDecimal(100)));
					...
					task.setFinishAmount(issueAmount);//已发放金额
