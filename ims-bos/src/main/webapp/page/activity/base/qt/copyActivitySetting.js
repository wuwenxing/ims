...
// 初始化
$(function() {
	copyActivitySetting.init();
});

var copyActivitySetting = {
	...
	save : function() {
		// 验证：活动基础设置与参与活动条件
		if (!common.submitFormValidate(copyActivitySetting.addDlogFormId)) {
			return false;
		}
		var url = BASE_PATH + "ActivitySettingController/save/qt";
		$.ajax({
			url : url,
			data : $("#" + copyActivitySetting.addDlogFormId).serialize(),
