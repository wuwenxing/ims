...
// 初始化
$(function() {
	addActivitySetting.init();
});

var addActivitySetting = {
	...
	save : function() {
		// 验证：活动基础设置与参与活动条件
		if (!common.submitFormValidate(addActivitySetting.addDlogFormId)) {
			return false;
		}
		var url = BASE_PATH + "ActivitySettingController/save/qt";
		$.ajax({
			url : url,
			data : $("#" + addActivitySetting.addDlogFormId).serialize(),
