...
<script type="text/javascript" src="<%=request.getContextPath()%>/jsp/activity/base/rw/copyActivitySetting.js" charset="UTF-8"></script>
</head>
<body>
			...
			<table class="commonTable">
			 <tbody id="table_activity_baseInfo">
				...
				<tr>
					...
					<td>
						<input	type="text" name="activityName" style="width: 380px;" class="easyui-validatebox" value="${actSetting.activityName}"
						data-options="required:true,validType: 'length[0,100]'" />
					</td>
					<th>活动状态</th>
					<td>
				...
				<tr>
					...
					<th>活动完成时间</th>
					<td>
						<input class="easyui-numberbox" type="text" name="finishDays" id="finishDays" value="${actSetting.finishDays}"  maxlength="3" data-options="min:0,max:999"/>天
					</td>
				</tr>
				<tr>
					<th>代币有效期</th> 
					<td>
						<input class="easyui-numberbox" type="text" name="coinDays" id="coinDays" value="${actSetting.coinDays}" maxlength="3" data-options="min:0,max:999" />天
					</td>
					<th>发放方式</th> 
					<td colspan="3">
			    ...
			    <tbody id="table_activity_condition">
					...
					<td colspan="6" align="center"  height="40px"><b>参与活动条件</b>
					    <br/>
					    <span class="spanRed">(温馨提示：活动一旦审批通过且已开始，则不能修改活动的开始时间及参与条件（除黑白名单有权限的人可以修改外），请知悉！)</span>
					</td>
				 </tr>
				<tr>
					...
					<td>
					   <input type="hidden"  id="accountOnlyDDA" name="custActCondition.accountOnly" value="${custActCondition.accountOnly}"/>
					...
					<td>
					    <c:set var="accountLevel" value="${custActCondition.accountLevel},"/><!-- 尾号加个逗号，避免相关前缀 -->
						 <c:forEach var="aLevel" items="${ActAccountLevel}" >
					      ...
					      <input type="checkbox" <c:if test="${custActCondition.accountType=='type_demo'}">disabled="disabled"</c:if> name="custActCondition.accountLevel" value="${aLevel.dictCode}" <c:if test="${fn:contains(accountLevel,dictCode)}">checked="checked"</c:if>/>${aLevel.dictName}
					     </c:forEach>
				 ...
				 <th>参与活动的平台<span class="spanRed">*</span></th>
					<td>
					   <c:set var="platforms" value="${custActCondition.platformsCcy},"/><!-- 尾号加个逗号，避免相关前缀代号-->
					   <c:forEach var="pform" items="${ActPlatformCcy}" varStatus="vs">
					       <c:set var="dictCode" value="${pform.dictCode},"/>
					      <input type="checkbox" name="custActCondition.platformsCcy" value="${pform.dictCode}" <c:if test="${fn:contains(platforms,dictCode)}">checked="checked"</c:if>/>${pform.dictName}
					...
					<th>账号注册日期</th>
					<td colspan="3">
					    <input class="easyui-datetimebox" type="text" name="custActCondition.registerStartTime"   id="registerStartTimeStr" value="${custActCondition.registerStartTime}" data-options="editable:false,validType:'compareToEndNew[\'registerEndTimeStr\']'"/>
					        至
					    <input class="easyui-datetimebox" type="text" name="custActCondition.registerEndTime"   id="registerEndTimeStr" value="${custActCondition.registerEndTime}" data-options="editable:false,validType:'compareToStart[\'registerStartTimeStr\']'"/>
					</td>
				</tr>
				<tr>
				 <th>只允许白名单用户参加活动</th>
					...
					<td>
					    ...
					    <br/>
						<input type="file" name="file" id="file1" style="width:155px;"><span id="file1NameSpan"> </span>
			        	<input type="hidden" name="custActCondition.whiteListFilePath" id="file1Path" value=""/>
			        	<input type="hidden" name="custActCondition.whiteListFileName" id="file1Name" value=""/>
					...
					<td>
				    	<a class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-download'" href="<%=request.getContextPath()%>/template/list_white_black_template.xlsx"><spring:message code="common.click.download.template"/></a>
				    	<br/>				    	
						<input type="file" name="file" id="file2" style="width:155px;"><span id="file2NameSpan"></span>
			        	<input type="hidden" name="custActCondition.blackListFilePath" id="file2Path" value=""/>
			        	<input type="hidden" name="custActCondition.blackListFileName" id="file2Name" value=""/>
