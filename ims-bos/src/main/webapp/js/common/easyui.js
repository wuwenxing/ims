...
var easyui = {
	...
	/**
	 * 关联两个datetimebox，可以使得选择的开始时间总是小于等于最后时间
	 */
	combineDateTimeBox : function(startId, endId) {
		$('#' + startId).datetimebox({
			onSelect : function(startDate) {
				$('#' + endId).datetimebox('calendar').calendar({
		...
		$('#' + endId).datetimebox({
			onSelect : function(endDate) {
				$('#' + startId).datetimebox('calendar').calendar({
	...
	/**
	 * datetimebox，可以使得开始时间不能小于当前时间
	 */
	combineStartDateTimeBox : function(startId) {
		$('#' + startId).datetimebox('calendar').calendar({
			validator : function(day) {
				var timestamp = Date.parse(new Date());
				var lasttime = timestamp - 86400000;
	...
	/**
	 * datebox，可以使得开始时间不能小于当前时间
	 */
	combineStartDateBox : function(startId) {
		$('#' + startId).datebox('calendar').calendar({
			validator : function(day) {
				var timestamp = Date.parse(new Date());
				var lasttime = timestamp - 86400000;
				var last = new Date(lasttime);
