...
public class ActCashinRealController extends BaseController{
	...
	@Autowired
	private ActCashinRealDubboService actCustomerInfoDubboService;

		
	/**
	 * @api {get} /act/cashinreal/page 分页查询
	...
	public MisRespResult<List<ActCashinRealDTO>> pageList(@ModelAttribute ActCashinRealDTO dto) {
		try{
			dto.setCompanyId(getCompanyId());
			Map<String, Object> result = actCustomerInfoDubboService.findPageList(dto,getCompanyId());
			if(null != result){
				Object list = result.get("list");
				return MisRespResult.success(list != null ? (List<ActCashinRealDTO>)list : null,result.get("total"));
			}else{
