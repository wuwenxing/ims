...
@Controller
@RequestMapping("/mis/act/cashoutreal")
public class ActCashoutRealController extends BaseController{
	...
	@Autowired
	private MisActCashoutRealDubboService misCashoutRealDubboService;

		
	/**
	 * @api {get} /mis/act/cashoutreal/page 分页查询
	...
	public MisRespResult<List<ActCashoutRealVO>> pageList(@ModelAttribute ActCashoutRealVO vo) {
		try{
            vo.setCompanyId(getCompanyId());
			Map<String,Object> result = misCashoutRealDubboService.findPageList(vo,getCompanyId());
			if(result != null){
				Object list = result.get("list");
				return MisRespResult.success(list != null ? (List<ActCashoutRealVO>)list : null,result.get("total"));
			}else{
