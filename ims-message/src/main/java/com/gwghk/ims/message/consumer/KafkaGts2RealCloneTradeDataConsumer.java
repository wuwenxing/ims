...
//@Component
public class KafkaGts2RealCloneTradeDataConsumer extends AbstractKafkaGts2lDataConsumer {
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2RealCloneTradeDataConsumer.class);

	@Autowired
	private KafkaGts2CloneTradeDataHandle kafkaGts2TradeDataHandle;

	public KafkaGts2RealCloneTradeDataConsumer() {
		super();
		super.kafkaTopic = KafkaTopic.GTS2_REAL_CLONE_TRADE;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount, String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2-{}-交易数据消息:{},topic:{},offset:{}", ActEnvEnum.REAL.getValue(), message,
						topic, offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2TradeDataHandle.handleMessage(message, ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2-{}-交易数据消息处理失败。message --> {}, error --> {}", ActEnvEnum.REAL.getValue(),
							message, e);
					return;
				}
			}
	...
	protected String info() {
		return "消费GTS2-real-交易数据消息-t_clone_trade";
	}
}
