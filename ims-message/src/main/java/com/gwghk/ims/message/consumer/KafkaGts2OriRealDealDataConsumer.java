...
/**
 * 
 * @ClassName: KafkaGts2OriRealDealDataConsumer
 ...
 */
@Component
public class KafkaGts2OriRealDealDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriRealDealDataConsumer.class);
	...
	public KafkaGts2OriRealDealDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_REAL_DEAL;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2真实交易数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2OriDealDataHandle.handleMessage(message,ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2真实交易数据消息处理失败。message --> {}, error --> {}", message, e);
				}
			}
		} catch (Exception e) {
