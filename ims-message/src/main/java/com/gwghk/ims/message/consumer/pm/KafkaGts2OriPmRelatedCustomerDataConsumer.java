...
/**
 * 
* @ClassName: KafkaGts2PMRelatedCustomerDataConsumer
* @Description: GTS2(PM)-真实账户模拟账号关联数据同步
* @author lawrence
* @date 2018年1月9日
*
 */
//@Component
public class KafkaGts2OriPmRelatedCustomerDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriPmRelatedCustomerDataConsumer.class);

	@Autowired
	private KafkaGts2RelatedCustomerDataHandle kafkaGts2RelatedCustomerDataHandle;
		
	public KafkaGts2OriPmRelatedCustomerDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_PM_REAL_RELATED_CUSTOMER;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(PM)-真实账户模拟账号关联数据同步消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						//kafkaGts2RelatedCustomerDataHandle.handleMessage(message,CompanyEnum.pm.getCode(),ActEnvEnum.REAL.getValue());
	...
	protected String info() {
		return "GTS2(PM)-真实账户模拟账号关联数据同步消息-t_related_customer";
	}
}
