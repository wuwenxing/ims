...
/**
 * 
 * @ClassName: KafkaGts2CFRealSymbolDataConsumer
 * @Description: GTS2(CF)-真实交易产品数据同步
 * @author lawrence
 * @date 2017年8月17日
 *
 */
//@Component
public class KafkaGts2OriCfRealSymbolDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriCfRealSymbolDataConsumer.class);

	@Autowired
	private KafkaGts2OriSymbolDataHandle kafkaGts2SymbolDataHandle;
		
	public KafkaGts2OriCfRealSymbolDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_CF_REAL_SYMBOL;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(CF)-真实交易产品数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2SymbolDataHandle.handleMessage(message,CompanyEnum.cf.getCode(),ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(CF)-真实交易产品数据处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(CF)-真实交易产品数据消息-gts2symbol";
	}
}
