 ...
 * @Description: GTS2-CF真实数据字典同步到活动中心
 * @author lawrence
 * @date 2017年8月14日
 *
 */
//@Component
public class KafkaGts2OriCfRealBoDictDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriCfRealBoDictDataConsumer.class);

	@Autowired
	private KafkaGts2OriBoDictDataHandle kafkaGts2BoDictDataHandle;
		
	public KafkaGts2OriCfRealBoDictDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_CF_REAL_BO_DICT;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(CF)-真实数据字典消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2BoDictDataHandle.handleMessage(message,CompanyEnum.cf.getCode(),ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(CF)-真实数据字典数据消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(CF)-真实数据字典消息-t_bo_dict";
	}
}
