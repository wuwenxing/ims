...
/**
 * 
 * @ClassName: KafkaGts2OriFxRealAccountGroupDataConsumer
 * @Description: GTS2(FX)-真实账户平台组数据同步
 * @author lawrence
 * @date 2017年8月17日
 *
 */
//@Component
public class KafkaGts2OriFxRealAccountGroupDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriFxRealAccountGroupDataConsumer.class);

	@Autowired
	private KafkaGts2OriAccountGroupDataHandle kafkaGts2OriAccountGroupDataHandle;
		
	public KafkaGts2OriFxRealAccountGroupDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_FX_REAL_ACCOUNT_GROUP;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(FX)-真实账号平台组数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2OriAccountGroupDataHandle.handleMessage(message,CompanyEnum.fx.getCode(),ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(FX)-真实账号平台组数据消息消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(FX)-真实账号平台组数据消息-t_account_group";
	}
}
