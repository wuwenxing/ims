...
/**
 * 
* @ClassName: KafkaGts2OriFxRelatedCustomerDataConsumer
* @Description: GTS2(FX)-真实账户模拟账号关联数据同步
* @author lawrence
* @date 2018年1月9日
*
 */
//@Component
public class KafkaGts2OriFxRelatedCustomerDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriFxRelatedCustomerDataConsumer.class);

	@Autowired
	private KafkaGts2RelatedCustomerDataHandle kafkaGts2RelatedCustomerDataHandle;
		
	public KafkaGts2OriFxRelatedCustomerDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_FX_REAL_RELATED_CUSTOMER;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(FX)-真实账户模拟账号关联数据同步消息:{},topic:{},offset:{}",message,topic,offset);
				try {
	...
	protected String info() {
		return "GTS2(FX)-真实账户模拟账号关联数据同步消息-t_related_customer";
	}
}
