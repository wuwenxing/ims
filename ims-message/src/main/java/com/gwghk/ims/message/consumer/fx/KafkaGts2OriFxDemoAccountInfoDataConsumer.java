...
/**
 * 
 * @ClassName: KafkaGts2OriFxDemoAccountInfoDataConsumer
 * @Description: GTS2(FX)-模拟账号平台数据同步
 * @author lawrence
 * @date 2017年8月17日
 *
 */
//@Component
public class KafkaGts2OriFxDemoAccountInfoDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriFxDemoAccountInfoDataConsumer.class);

	@Autowired
	private KafkaGts2OriAccountInfoDataHandle kafkaGts2OriAccountInfoDataHandle;
		
	public KafkaGts2OriFxDemoAccountInfoDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_FX_DEMO_ACCOUNT;
	}
	...
	protected void handlerPartition(final KafkaStream<String,String> stream,int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(FX)-模拟账号平台数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2OriAccountInfoDataHandle.handleMessage(message,CompanyEnum.fx.getCode(),ActEnvEnum.DEMO.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(FX)-模拟账号平台数据消息消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(FX)-模拟账号平台数据消息-t_account_info";
	}
}
