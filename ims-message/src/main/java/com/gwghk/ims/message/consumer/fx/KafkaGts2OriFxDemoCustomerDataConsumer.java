...
/**
 * 
 * @ClassName: KafkaGts2OriFxDemoCustomerDataConsumer
 * @Description: GTS2-FX模拟客户账号资料数据同步到活动中心
 * @author lawrence
 * @date 2017年8月14日
 *
 */
@Component
public class KafkaGts2OriFxDemoCustomerDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriFxDemoCustomerDataConsumer.class);

	@Autowired
	private KafkaGts2OriCustomerDataHandle kafkaGts2OriCustomerDataHandle;
		
	public KafkaGts2OriFxDemoCustomerDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_FX_DEMO_CUSTOMER;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(FX)-模拟账号基础资料数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2OriCustomerDataHandle.handleMessage(message,CompanyEnum.fx.getCode(),ActEnvEnum.DEMO.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(FX)-模拟账号基础资料数据消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(FX)-模拟账号基础资料数据消息-t_customer";
	}
}
