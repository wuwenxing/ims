...
//@Component
public class KafkaGts2RealCloneCustomerAccountInfoDataConsumer extends AbstractKafkaGts2lDataConsumer {
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2RealCloneCustomerAccountInfoDataConsumer.class);

	@Autowired
	private KafkaGts2CloneCustomerAccountInfoDataHandle kafkaGts2CustomerAccountInfoDataHandle;

	public KafkaGts2RealCloneCustomerAccountInfoDataConsumer() {
		super();
		super.kafkaTopic = KafkaTopic.GTS2_REAL_CLONE_CUSTOMER_ACCOUNT_INFO;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount, String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2-{}-平台客户资料数据消息:{},topic:{},offset:{}", ActEnvEnum.REAL.getValue(),
						message, topic, offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2CustomerAccountInfoDataHandle.handleMessage(message, ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2-{}-平台客户资料数据消息处理失败。message --> {}, error --> {}",
							ActEnvEnum.REAL.getValue(), message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "消GTS2-real-平台客户资料数据消息-t_clone_customer_account_info";
	}
}
