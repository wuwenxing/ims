...
/**
 * 
 * @ClassName: KafkaGts2HXRealAccountInfoDataConsumer
 * @Description: GTS2(HX)-真实账号平台数据同步
 * @author lawrence
 * @date 2017年8月17日
 *
 */
//@Component
public class KafkaGts2OriHxRealAccountInfoDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriHxRealAccountInfoDataConsumer.class);

	@Autowired
	private KafkaGts2OriAccountInfoDataHandle kafkaGts2AccountInfoDataHandle;
		
	public KafkaGts2OriHxRealAccountInfoDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_HX_REAL_ACCOUNT;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(HX)-真实账号平台数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2AccountInfoDataHandle.handleMessage(message,CompanyEnum.hx.getCode(),ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(HX)-真实账号平台数据消息消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(HX)-真实账号平台数据消息-t_account_info";
	}
}
