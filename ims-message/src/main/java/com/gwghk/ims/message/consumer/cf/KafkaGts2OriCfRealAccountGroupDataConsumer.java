 ...
 * @Description: GTS2(CF)-真实账户平台组数据同步
 * @author lawrence
 * @date 2017年8月17日
 *
 */
//@Component
public class KafkaGts2OriCfRealAccountGroupDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriCfRealAccountGroupDataConsumer.class);

	@Autowired
	private KafkaGts2OriAccountGroupDataHandle kafkaGts2AccountGroupDataHandle;
		
	public KafkaGts2OriCfRealAccountGroupDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_CF_REAL_ACCOUNT_GROUP;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(CF)-真实账号平台组数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2AccountGroupDataHandle.handleMessage(message,CompanyEnum.cf.getCode(),ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(CF)-真实账号平台组数据消息-t_account_group";
	}
}
