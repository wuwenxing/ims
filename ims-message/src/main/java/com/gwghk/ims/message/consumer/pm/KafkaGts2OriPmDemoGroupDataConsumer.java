 ...
 * @Description: GTS2(PM)-模拟交易账号组数据同步
 * @author lawrence
 * @date 2017年8月18日
 *
 */
//@Component
public class KafkaGts2OriPmDemoGroupDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriPmDemoGroupDataConsumer.class);

	@Autowired
	private KafkaGts2OriGroupDataHandle kafkaGts2GroupDataHandle;
		
	public KafkaGts2OriPmDemoGroupDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_PM_DEMO_GROUP;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(PM)-模拟交易账号组数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2GroupDataHandle.handleMessage(message,CompanyEnum.pm.getCode(),ActEnvEnum.DEMO.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(PM)-模拟交易账号组数据消息消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(PM)-模拟交易账号组数据消息-gts2group";
	}
}
