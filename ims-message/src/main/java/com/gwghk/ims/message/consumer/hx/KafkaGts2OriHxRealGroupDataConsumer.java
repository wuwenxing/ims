 ...
 * @Description: GTS2(HX)-真实交易账号组数据同步
 * @author lawrence
 * @date 2017年8月18日
 *
 */
//@Component
public class KafkaGts2OriHxRealGroupDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriHxRealGroupDataConsumer.class);

	@Autowired
	private KafkaGts2OriGroupDataHandle kafkaGts2GroupDataHandle;
		
	public KafkaGts2OriHxRealGroupDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_HX_REAL_GROUP;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(HX)-真实交易账号组数据消息:{},offset:{}",message,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2GroupDataHandle.handleMessage(message,CompanyEnum.hx.getCode(),ActEnvEnum.REAL.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(HX)-真实交易账号组数据消息消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(HX)-真实交易账号组数据消息-gts2group";
	}
}
