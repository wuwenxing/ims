...
/**
 * 
 * @ClassName: KafkaGts2CFDemoGroupDataConsumer
 * @Description: GTS2(CF)-模拟交易账号组数据同步
 * @author lawrence
 * @date 2017年8月18日
 *
 */
//@Component
public class KafkaGts2OriCfDemoGroupDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriCfDemoGroupDataConsumer.class);

	@Autowired
	private KafkaGts2OriGroupDataHandle kafkaGts2GroupDataHandle;
		
	public KafkaGts2OriCfDemoGroupDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_CF_DEMO_GROUP;
	}
	...
	protected void handlerPartition(final KafkaStream<String, String> stream, int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(CF)-模拟交易账号组数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2GroupDataHandle.handleMessage(message,CompanyEnum.cf.getCode(),ActEnvEnum.DEMO.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(CF)-模拟交易账号组数据消息消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(CF)-模拟交易账号组数据消息-gts2group";
	}
}
