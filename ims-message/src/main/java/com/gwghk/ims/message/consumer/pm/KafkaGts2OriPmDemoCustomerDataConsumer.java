...
/**
 * 
 * @ClassName: KafkaGts2PMDemoCustomerDataConsumer
 * @Description: GTS2-PM模拟客户账号资料数据同步到活动中心
 * @author lawrence
 * @date 2017年8月14日
 *
 */
//@Component
public class KafkaGts2OriPmDemoCustomerDataConsumer extends AbstractKafkaGts2lDataConsumer{
	private final static Logger logger = LoggerFactory.getLogger(KafkaGts2OriPmDemoCustomerDataConsumer.class);

	@Autowired
	private KafkaGts2OriCustomerDataHandle kafkaGts2CustomerDataHandle;
		
	public KafkaGts2OriPmDemoCustomerDataConsumer(){
		super();
		super.kafkaTopic = KafkaTopic.GTS2_PM_DEMO_CUSTOMER;
	}
	...
	protected void handlerPartition(final KafkaStream<String,String> stream,int retryCount,String topic) {
		try {
			...
			while (streamIterator.hasNext()) {
				...
				logger.info("handlerPartition->消费GTS2(PM)-模拟账号基础资料数据消息:{},topic:{},offset:{}",message,topic,offset);
				try {
					if (StringUtil.isNotEmpty(message)) {
						kafkaGts2CustomerDataHandle.handleMessage(message,CompanyEnum.pm.getCode(),ActEnvEnum.DEMO.getValue());
					}
				} catch (Exception e) {
					logger.error("kafka消费GTS2(PM)-模拟账号基础资料数据消息处理失败。message --> {}, error --> {}", message, e);
					return;
				}
			}
		} catch (Exception e) {
	...
	protected String info() {
		return "GTS2(PM)-模拟账号基础资料数据消息-t_customer";
	}
}
